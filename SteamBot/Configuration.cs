...
namespace SteamBot
{
    ...
    {        
        public static Configuration LoadConfiguration (string filename)
        {
            ...
            config.CheckinCheckoutUsers = config.CheckinCheckoutUsers ?? new ulong[0];
            ...
            foreach (BotInfo bot in config.Bots)
            {
                ...
                if (bot.CheckinCheckoutUsers == null)
                {
                    bot.CheckinCheckoutUsers = new ulong[config.CheckinCheckoutUsers.Length];
                    Array.Copy(config.CheckinCheckoutUsers, bot.CheckinCheckoutUsers, config.CheckinCheckoutUsers.Length);
                }
                else
                {
                    bot.CheckinCheckoutUsers = bot.CheckinCheckoutUsers.Concat(config.CheckinCheckoutUsers).ToArray();
                }
                if (string.IsNullOrEmpty(bot.ApiKey))
                {
        ...
        /// <summary>
        /// Gets or sets the users who can checkin or checkout items.
        /// </summary>
        /// <value>
        /// An array of Steam Profile IDs (64 bit IDs) of the users. Each Profile ID should be a string in quotes 
        /// and separated by a comma. These admins are global to all bots 
        /// listed in the Bots array.
        /// </value>
        public ulong[] CheckinCheckoutUsers { get; set; }
